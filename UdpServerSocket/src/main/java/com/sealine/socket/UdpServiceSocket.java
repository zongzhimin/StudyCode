...
public class UdpServiceSocket {
			...
			public void run() {
				try {
					...
					while(flag){
						...
						String shipName = new String(p.getData(),"UTF-8").trim();//船名
						Date d = new Date();
						System.out.println(d+"接收到船名"+shipName);
	...
	public void send(final String shipName,final String message){
		if(null==socket){
		...
		new Thread(){
			...
			public void run() {
				try{
					...
					DatagramPacket dp = new DatagramPacket(message.getBytes("UTF-8"), message.getBytes("UTF-8").length,
